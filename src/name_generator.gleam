...
const adjectives = [
  ...
  "confident", "cool", "cranky", "crazy", "dazzling", "determined", "distracted",
...
fn adjectives_generator() {
  random.uniform("admiring", adjectives)
}

fn surnames_generator() {
  random.uniform("agnesi", surnames)
}

fn docker_names_generator() {
  random.map2(adjectives_generator(), surnames_generator(), fn(adj, surname) {
    adj <> "_" <> surname
  })
}

/// Generate a Docker-style random name like "clever_turing"
/// and thread the seed through for purity/reproducibility.
pub fn generate_name(s: seed.Seed) -> #(String, seed.Seed) {
  random.step(docker_names_generator(), s)
}

pub fn generate_random_name() -> String {
  random.random_sample(docker_names_generator())
}
